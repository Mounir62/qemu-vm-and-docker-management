...
ContainerPilot API
──────────────────
FastAPI backend that controls QEMU VMs, virtual disks and Docker resources.
All routes are mounted under /api and return/accept JSON exclusively.

Changes in this revision
────────────────────────
✓ Unified subprocess execution through the `_run()` helper.
✓ Graceful 503 errors when the Docker CLI is missing or the daemon is stopped.
✓ Clear stderr bubbling for *all* shell commands.
✓ Minor refactors / type-hints for readability.

Test matrix
───────────
docker version              → OK
curl :8000/api/docker/images→ {} list
curl :8000/api/disks        → [] list
"""
...
DOCKER_BIN: str | None = shutil.which("docker")  # resolved once at startup
...
def _run(cmd: list[str]) -> str:
    ...
    • If command is 'docker', validates the CLI path & daemon connectivity.
    • Raises `HTTPException` with the correct status on error.
    • Returns *stdout* as text.
    """
    # Fix Windows PATH / give nicer error if docker missing
    if cmd[0] == "docker":
...
def list_images():
    ...
    stdout = _run(["docker", "images", "--format", fmt])
...
def search_local_images(req: ImageSearch):
    ...
    term = req.term.lower()
    return {"results": [i for i in imgs if term in i["repository"].lower()]}
